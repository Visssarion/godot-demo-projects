...
func generate_polygon():
	...
	# Increasie bitmap to make sure every pixel will be captured
	bitmap.grow_mask(1, cell_rect)
	# Generating array of polygons from bitmap
	...
	# I dont use a camera but if i did i would have to offset += camera.position
